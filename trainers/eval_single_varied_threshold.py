...
def test(models, writer, criterion, data_loader, epoch):
    ...
    with torch.no_grad():
        ...
        for data, target in val_loader:
            ...
            # need log-probabilities here to correctly calculate predictions, 
            # and we're doing binary classification, so only need probabilities for one class
            output_probs = torch.exp(F.log_softmax(output, dim=1)[:,0])
            target = (~target.to(bool)).to(int)
            test_loss += criterion(output, target).item()

            # get the index of the max log-probability
            # might need to flip > if should go in the other direction
            pred = (output_probs < args.alpha0).to(int)
